...
sys.path.append('./cell_gears_local')
from pertdata_local import PertData
from gears_modified import GEARS
from gears_modified import compute_metrics, deeper_analysis, non_dropout_analysis
from gears_modified import create_cell_graph_dataset_for_prediction
sys.path.insert(0, "../")
...
adata = sc.read_h5ad('./data/updated_file.h5ad')
adata = adata.concatenate(adata.copy())
adata.X = sparse.csr_matrix(adata.X)
print(adata.X.toarray().shape)
pert_data = PertData(data_path='./data')
cluster_counts = adata.obs['condition'].value_counts()
keep = cluster_counts.index[cluster_counts >1]
adata = adata[adata.obs['condition'].isin(keep)]
print(adata.obs['condition'].tolist())
pert_data.new_data_process(dataset_name = 'doris', adata = adata)
pert_data.load(data_path = './data/doris')
sys.path.append('./cell_gears_local')
pert_data.prepare_split(split='simulation', seed=1, train_gene_set_size=0.90, combo_seen2_train_frac=0.1) # adjust these values
...
def compute_perturbation_metrics_local(
    results: Dict,
    ctrl_adata: AnnData,
...
for epoch in range(1, epochs + 1):
    ...
    val_metrics = compute_perturbation_metrics_local(
